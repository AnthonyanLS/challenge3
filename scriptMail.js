...
/*function postRecord() {
    ...
    xhttp.open("POST", `${URL}?firstName=${firstName}&lastName=${lastName}&email=${email}`, true);
    xhttp.send();
...
function postRecordAjax() {
    ...
    $.post({
        url: `${URL}?firstName=${firstName}&lastName=${lastName}&email=${email}`
    });
}
...
function getRecord() {
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            var response = JSON.parse(this.responseText);
            for (var i = 0; i < response.length; i++) {
                document.getElementById("result").innerHTML += response[i].firstName + " | " + response[i].lastName + " | " + response[i].email + "<br> ";
            }
        }
    };
